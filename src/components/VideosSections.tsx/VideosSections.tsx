...
const VideosSections = ({ data, loading } : { data: VideoResponse | null, loading: boolean }) => {
    ...
    return (
      <div className="videosSection relative mb-12 mb-12 bg-[#04152d] ">
        ...
        <ContentWrapper className2="max-w-screen-2xl">
          {!loading ? (
            <div className="relative flex flex-row me-5 md:me-0">
                ...
                <div className="videos flex gap-2.5 overflow-y-hidden overflow-x-hidden -mx-5 px-5 md:gap-5 md:mx-0 md:px-0"
                    ref={listRef}>

                    {data?.results?.map((video) => (
                        <div key={video.id} className="videoItem w-[150px] flex-shrink-0 cursor-pointer md:w-1/4">
                        <div
            ...
            <div className="videoSkeleton flex gap-2.5 overflow-x-auto -mx-5 px-5 md:gap-5 md:mx-0 md:px-0">
              {loadingSkeleton()}
              {loadingSkeleton()}
              {loadingSkeleton()}
              {loadingSkeleton()}
